...
| Intake storage/review | Public route stores normalized submissions and still starts a draft generation run through `processStoredIntakeSubmission`. Generic self-serve provider-backed block variants are currently disabled, so generation remains blocked until exact-source Tailwind Plus variants are reintroduced. The reviewed-intake super-admin controls remain a recovery path for failed or imported submissions that do not already have a generation run. |
