...
| Intake storage/review | Public route stores normalized submissions and starts a draft generation run through `processStoredIntakeSubmission`. Generic self-serve generation is active only for approved exact-source Tailwind Plus Marketing provider surfaces: page sections backed by the executable source-block registry (hero, feature sections, CTA, contact, testimonial, stats, logo-cloud, pricing, team, newsletter, bento grid, content section, blog/cards) and header/banner chrome backed by the executable source-chrome registry. The known-tenant 404 fallback is provider-backed in the renderer, not generated page content. The reviewed-intake super-admin controls remain a recovery path for failed or imported submissions that do not already have a generation run. |
