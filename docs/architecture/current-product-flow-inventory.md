...
| Intake storage/review | Public route stores normalized submissions and starts a draft generation run through `processStoredIntakeSubmission`. Generic self-serve generation may select only explicit variants from the pinned `akash3444/shadcn-ui-blocks` canonical manifest. Page blocks, chrome, and known-tenant 404 templates are structured provider-backed data. The reviewed-intake super-admin controls remain a recovery path for failed or imported submissions that do not already have a generation run. |
