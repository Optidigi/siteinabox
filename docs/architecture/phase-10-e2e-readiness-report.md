...
- Publish/activate/rollback, activation gates, immutable snapshots, draft/live isolation, inactive tenant behavior, and alias host resolution are covered: `apps/cms/tests/unit/publish-current-state.test.ts`, `apps/cms/tests/unit/publish-route.test.ts`, `apps/cms/tests/unit/publishOperations.test.ts`.
