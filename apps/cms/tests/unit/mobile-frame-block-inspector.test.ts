...
describe("mobile iframe native editor source contract", () => {
  it("hosts parent-owned section list, focused frame, and two-detent field inspector", () => {
    ...
    expect(shell).toContain("const focusedBlock = api.blocks[focusedIndex]")
    expect(shell).toContain("active={isSectionScreen}")
    expect(shell).toContain('aria-hidden={!active}')
    expect(shell).toContain('"fixed left-0 top-0 h-px w-px overflow-hidden opacity-0 pointer-events-none"')
