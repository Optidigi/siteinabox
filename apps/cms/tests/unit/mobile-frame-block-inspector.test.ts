...
describe("mobile iframe native editor source contract", () => {
  it("hosts the canonical parent-owned section list, focused frame, and two-detent inspector", () => {
    ...
    expect(shell).toContain('className="-mx-4 flex h-[calc(100dvh-4.5rem)] min-h-[calc(100dvh-4.5rem)] flex-col"')
    expect(shell).toContain('className="relative flex min-h-0 flex-1 flex-col overflow-hidden"')
    expect(shell).toContain('className="absolute inset-0 min-h-0"')
    expect(shell).not.toContain('className="flex h-[calc(100dvh-4.5rem)] min-h-0 flex-col"')
    expect(shell).not.toContain('<div className="min-h-0">')
