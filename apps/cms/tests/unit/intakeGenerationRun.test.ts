...
describe("processIntakeSubmission", () => {
  it("creates intake and generation-run records and applies mocked draft CMS data", async () => {
    ...
    expect(store.pages[0]?.blocks[1]?.logos.every((logo: any) => !("image" in logo))).toBe(true)
    ...
    expect(store.pages[0]?.blocks[9]?.members.every((member: any) => !("image" in member))).toBe(true)
    expect(store.pages[0]?.blocks[10]?.posts).toHaveLength(3)
    expect(store.pages[0]?.blocks[10]?.posts.every((post: any) => !("image" in post))).toBe(true)
