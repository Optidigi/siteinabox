...
describe("canvas chrome fidelity", () => {
  ...
  it("portals editor-owned canvas controls outside tenant-scoped .rt-canvas styling", () => {
    ...
    expect(canvasMode).toContain("${overlayPosition.className} fixed z-[15]")
    ...
    expect(read("../../packages/site-renderer/src/canvas.css")).toContain(".site-renderer[data-siab-site-renderer] .rt-canvas :is([data-site-chrome], [data-amicare-nav], .site-frame-root > nav, .site-frame-root > footer)")
