...
describe("SiteChromePreview chrome actions", () => {
  ...
  it("opens chrome menus from right-click or the explicit options button, not ordinary footer clicks", () => {
    ...
    expect(gutterOverlay).toContain("onPointerDown={(event) => {")
    expect(gutterOverlay).toContain("if (event.button !== 0) return")
    ...
    expect(gutterOverlay).toContain("element.closest(\".rt-canvas, [data-siab-canvas-chrome]\")")
  ...
  it("keeps shared renderer chrome DOM direct and uses a portal overlay target for edit affordances", () => {
    ...
    expect(file).toContain("const setGutterVisibleSafely = (next: boolean) => {")
    expect(file).toContain("}, 250)")
