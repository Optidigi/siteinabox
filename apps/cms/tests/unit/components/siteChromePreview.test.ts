...
describe("SiteChromePreview chrome actions", () => {
  ...
  it("opens chrome menus at the iframe click point for site chrome targets", () => {
    ...
    expect(file).not.toContain("hoverSyncInterval")
    expect(file).not.toContain("syncHoveredTarget")
  ...
  it("keeps shared renderer chrome DOM direct and uses a portal overlay target for edit affordances", () => {
    ...
    expect(file).toContain("setGutterVisible(next)")
    expect(file).not.toContain("gutterHideTimerRef")
    expect(file).not.toContain("hideDelayMs")
  ...
  it("disables canvas context menus in sidebar mode", () => {
    ...
    expect(canvasMode).toContain("const effectiveReadOnly = readOnly || isReadOnlyView(view) || !allowInlineEditing")
    expect(canvasMode).toContain("if (effectiveReadOnly || !showGutters) {")
