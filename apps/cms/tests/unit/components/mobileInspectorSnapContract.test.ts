...
describe("mobile inspector snap contract", () => {
  ...
  it("keeps mobile section navigation and save affordances pill-shaped", () => {
    ...
    expect(frameEditor).toContain("window.scrollTo({ top: 0 })")
    expect(frameEditor).toContain('className="-mx-4 flex h-[calc(100dvh-4.5rem)] min-h-[calc(100dvh-4.5rem)] flex-col"')
    ...
    expect(frameEditor).toContain('className="sticky top-0 z-30 flex min-w-0 shrink-0 items-center justify-center border-b border-border bg-background px-16 py-3"')
    expect(frameEditor).toContain('className="relative flex min-h-0 flex-1 flex-col overflow-hidden"')
    expect(frameEditor).toContain('className="absolute inset-0 min-h-0"')
    expect(frameEditor).toContain('className="size-11 rounded-full"')
    expect(frameEditor).toContain('variant="outline"')
    expect(frameEditor).toContain("rounded-full border-border bg-muted px-4 font-medium shadow-sm")
    expect(frameEditor).toContain('<span className="truncate">{label}</span>')
    ...
    expect(frameEditor).toContain("const [trashPillVisible, setTrashPillVisible] = React.useState(true)")
    expect(frameEditor).toContain("setTrashPillVisible(window.scrollY <= 24)")
    expect(frameEditor).toContain('window.addEventListener("scroll", onScroll, { passive: true })')
    expect(frameEditor).toContain("isInspectorIdle && (")
    expect(frameEditor).toContain('position="top-right"')
    expect(frameEditor).toContain('offset="3.75rem"')
    expect(frameEditor).toContain("visible={trashPillVisible}")
    ...
    expect(floatingPill).toContain("visible")
