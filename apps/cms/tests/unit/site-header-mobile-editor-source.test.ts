...
describe("site header mobile page editor chrome", () => {
  it("hides the sidebar nav trigger on mobile page editor routes only", () => {
    ...
    expect(siteHeader).toContain('/^\\/pages\\/(?:new|\\d+)$/.test(pathname)')
    expect(siteHeader).toContain('/^\\/sites\\/[^/]+\\/pages\\/(?:new|\\d+)$/.test(pathname)')
    expect(siteHeader).toContain("const hideMobileNavTrigger = isPageEditorPath(pathname)")
