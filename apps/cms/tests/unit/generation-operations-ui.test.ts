...
describe("generation operations route access", () => {
  ...
  it("exposes Phase 8 snapshot lifecycle controls behind super-admin server actions", () => {
    ...
    const visiblePanel = detail.slice(detail.indexOf('title="Send preview"'), detail.indexOf("<CardTitle>Advanced"))
    ...
    expect(visiblePanel).toContain('title="Send preview"')
    expect(visiblePanel).toContain('title="Client"')
    expect(visiblePanel).toContain("<CardTitle>Status")
    expect(detail).toContain('label: "Checkout"')
    expect(detail).toContain('label: "Provisioning"')
    expect(detail).toContain('label: "Live"')
  ...
  it("presents the detail page as a manager-facing draft-site workflow", () => {
    ...
    expect(detail).toContain('title="Client"')
    expect(detail).toContain("<CardTitle>Status")
    expect(detail).toContain("Preview email")
    expect(detail.indexOf('title="Send preview"')).toBeLessThan(detail.indexOf('title="Client"'))
    expect(detail.indexOf('title="Send preview"')).toBeLessThan(detail.indexOf("<CardTitle>Advanced"))
