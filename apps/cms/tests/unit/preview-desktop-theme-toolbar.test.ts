...
describe("preview desktop theme toolbar", () => {
  it("renders inline theme controls for desktop preview without popovers", () => {
    ...
    expect(toolbar).toContain("BackgroundModeControl")
    expect(toolbar).toContain('layout="segment"')
    ...
    expect(inlineGroup).toContain("neoTray")
    expect(read("src/components/editor/theme/font-picker.tsx")).toContain('resolvedSize === "segment"')
    expect(inlineGroup).toContain('variant={active ? "default" : "outline"}')
    ...
    expect(customizer).toContain('const checkoutHref = `/${access.clientSlug}/checkout`')
    expect(customizer).toContain('const reviewHref = `/${access.clientSlug}/review`')
    expect(customizer).toContain('aria-label={t("reviewChanges")}')
    expect(customizer).toContain('aria-label={t("launchWebsite")}')
    expect(customizer).not.toContain("showCustomerActions")
    expect(customizer).not.toContain('access.clientSlug !== "sitegen-review"')
