...
describe("renderer iframe source contract", () => {
  ...
  it("uses the shared iframe-editor protocol for customer preview messages", () => {
    ...
    expect(contracts).not.toContain('"consent.preview"')
    ...
    expect(frameRuntime).toContain("createPreviewConsentRuntime")
    expect(frameRuntime).toContain("initializeConsentBehavior")
