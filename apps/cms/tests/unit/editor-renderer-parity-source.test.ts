...
describe("page editor renderer parity", () => {
  ...
  it("uses parent scroll with renderer.height sync and parent consent overlay", () => {
    const runtime = read("src/components/editor-frame/EditorFrameRuntime.tsx")
    const host = read("src/components/editor/iframe/PageEditorFrameHost.tsx")
    const form = read("src/components/forms/PageForm.tsx")
    ...
    expect(runtime).toContain("parentScroll && isViewportFixedConsentBanner(frameSettings) && showChrome")
    expect(runtime).toContain("renderer-frame-preview-viewport")
    expect(runtime).toContain("isViewportFixedConsentBanner")
    ...
    expect(host).toContain("<EditorConsentBannerOverlay")
    ...
    expect(siabCss).toContain("[data-siab-editor-consent-overlay]")
    expect(siabCss).toContain("outline-offset: 2px")
    expect(siabCss).toContain("box-shadow: inset 3px 0 0 0 var(--siab-ed-ink)")
