...
describe("page editor renderer parity", () => {
  ...
  it("uses iframe-internal scroll with fixed cookie parity like preview", () => {
    const runtime = read("src/components/editor-frame/EditorFrameRuntime.tsx")
    const host = read("src/components/editor/iframe/PageEditorFrameHost.tsx")
    const form = read("src/components/forms/PageForm.tsx")
    const css = read("src/styles/generated-site-renderer.css")
    expect(runtime).not.toContain('type: "renderer.height"')
    expect(runtime).not.toContain("new ResizeObserver(reportHeight)")
    expect(runtime).toContain('data-siab-preview-viewport="true"')
    expect(runtime).toContain("renderer-frame-preview-viewport")
    expect(host).not.toContain('message.type === "renderer.height"')
    expect(host).not.toContain('scrolling="no"')
    expect(host).toContain("h-full min-h-0 w-full overflow-hidden")
    expect(host).toContain("h-full min-h-0")
    expect(host).not.toContain("min-h-[32rem]")
    expect(host).not.toContain("min-h-[640px]")
    expect(form).toContain("h-[calc(100dvh-6.5rem)]")
    expect(runtime).toContain("window.innerHeight")
    expect(runtime).not.toContain("hostWindow.innerHeight")
    const mobileFrame = read("src/components/editor/iframe/MobileFrameEditor.tsx")
    expect(mobileFrame).toContain("flex min-h-0 flex-1 flex-col overflow-hidden")
    expect(css).not.toContain("data-siab-editor-parent-scroll")
