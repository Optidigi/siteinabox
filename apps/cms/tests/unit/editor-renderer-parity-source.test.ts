...
describe("page editor renderer parity", () => {
  ...
  it("reveals the iframe only after renderer, load, fonts, and layout are ready", () => {
    ...
    expect(host).toContain("PreviewFrameLoading")
  })
  ...
  it("waits for breakpoint resolution before mounting mobile or desktop editor chrome", () => {
    ...
    expect(core).not.toContain("mobileFocusedSectionIndex")
  ...
  it("resyncs iframe snapshot revision on ready and rejects only stale revisions", () => {
    ...
    expect(editorRuntime).toContain("CanvasPageSchema.safeParse")
    ...
    expect(rendererRuntime).toContain("CanvasPageSchema.safeParse")
  ...
  it("scrolls canvas only when host sets revealSelection on snapshot", () => {
    ...
    expect(runtime).toContain("function scrollWithinFrame")
    expect(runtime).not.toContain("blockNode.scrollIntoView")
  ...
  it("normalizes canvas wire page/settings before live snapshots", () => {
    ...
    expect(wire).toContain("CanvasPageSchema.safeParse")
    expect(wire).toContain("BlockSchema.safeParse")
    expect(wire).toContain("language.trim()")
    expect(wire).toContain('"nl"')
    expect(form).toContain("ensureCanvasWirePage(pageToJson(")
    expect(form).toContain("ensureCanvasWireSettings(settingsToJsonWithoutAnalytics")
    expect(framePage).toContain("ensureCanvasWireSettings(stripCanvasConsent(")
  })

  it("uses parent scroll with renderer.height sync without unimplemented chrome", () => {
    const runtime = read("src/components/editor-frame/EditorFrameRuntime.tsx")
    const host = read("src/components/editor/iframe/PageEditorFrameHost.tsx")
    const form = read("src/components/forms/PageForm.tsx")
    ...
    expect(runtime).toContain("if (!parentScroll || !readyForBlockShape) return")
    expect(runtime).not.toContain("isViewportFixedConsentBanner")
    expect(runtime).not.toContain("suppressInFrameConsentBanner")
    expect(runtime).not.toContain("banner={")
    expect(runtime).not.toContain("data-siab-navbar")
    expect(runtime).not.toContain("data-siab-footer")
    ...
    expect(host).toContain("min-h-[32rem]")
    ...
    expect(form).toContain('sticky top-[calc(7.5rem+0.5rem)] h-[calc(100dvh-7.5rem)]')
    ...
    const mobileFrame = read("src/components/editor/iframe/MobilePageEditorShell.tsx")
    expect(mobileFrame).toContain("snap-x snap-mandatory")
    expect(mobileFrame).toContain("overflow-x-auto overflow-y-hidden")
    expect(css).toContain("data-siab-editor-parent-scroll")
    expect(css).not.toContain("data-siab-cookie-consent")
    expect(css).not.toContain("data-site-chrome")
  ...
  it("keeps the tenant canvas free of operator UI so Ami Care and other live sites stay on one renderer", () => {
    const manifest = readRepo("packages/site-renderer/package.json")
    const cn = readRepo("packages/site-renderer/src/cn.ts")
    expect(manifest).not.toContain("@siteinabox/ui")
    expect(cn).toContain("export function cn(")
    expect(readRepo("packages/site-renderer/src/SitePageShell.tsx")).toContain('import { cn } from "./cn"')
    expect(readRepo("packages/site-renderer/src/SitePageShell.tsx")).not.toContain("@siteinabox/ui")
  })
})
