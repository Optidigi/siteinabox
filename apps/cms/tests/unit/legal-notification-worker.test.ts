...
const createPayload = () => {
  ...
  const find = vi.fn(async ({ collection, where }: MockFindArgs) => {
    ...
    const key = (where as MockFindArgs["where"])?.notificationKey?.equals
  ...
  const update = vi.fn(async ({ collection, id: rowId, data }: MockUpdateArgs) => {
    ...
    if (!row) return rowId == null ? { docs: [] } : row
    Object.assign(row, data)
    return row
  })
  const findByID = vi.fn(async () => requirement)
  return { payload: asPayload({ find, findByID, create, update }), requirement, deliveries }
}
...
describe("legal notification worker", () => {
  ...
  it("sends once, records the logical delivery, and marks the requirement notified", async () => {
    ...
    await expect(processLegalRequirementNotifications({ payload: asPayload(payload), now })).resolves.toMatchObject({ sent: 1, failed: 0 })
    await expect(processLegalRequirementNotifications({ payload: asPayload(payload), now })).resolves.toMatchObject({ sent: 0, skipped: 1 })
  ...
  it("retains a failed requirement and schedules a retry", async () => {
    ...
    const now = new Date("2026-07-11T12:00:00.000Z")
    await expect(processLegalRequirementNotifications({ payload: asPayload(payload), now })).resolves.toMatchObject({ sent: 0, failed: 1 })
  ...
  it("emails a direct notice once without presenting an acceptance action", async () => {
    ...
    await processLegalRequirementNotifications({ payload: asPayload(payload), now: new Date("2026-07-11T12:00:00.000Z") })
  ...
  it("sends one reminder before the continued-use objection deadline", async () => {
    ...
    requirement.enforceAt = null
    await processLegalRequirementNotifications({ payload: asPayload(payload), now: new Date("2026-07-11T12:00:00.000Z") })
    await processLegalRequirementNotifications({ payload: asPayload(payload), now: new Date("2026-08-05T12:00:00.000Z") })
  ...
  it("does not send for an active lease", async () => {
    ...
    await expect(processLegalRequirementNotifications({ payload: asPayload(payload), now: new Date("2026-07-11T12:00:00.000Z") }))
