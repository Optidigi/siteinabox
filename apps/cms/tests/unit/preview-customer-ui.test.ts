...
describe("customer preview UI", () => {
  it("uses a dedicated customer preview canvas mode without editor metadata badges", () => {
    ...
    expect(customizer).toContain("size=\"icon\"")
    ...
    expect(customizer).toContain("pointer-events-auto grid w-full grid-cols-[1fr_auto_1fr] items-center gap-2")
    expect(customizer).toContain("md:mx-auto md:max-w-4xl md:rounded-lg")
    expect(customizer).toContain("size-12 justify-self-start")
    ...
    expect(customizer).not.toContain('md:not-sr-only md:ml-2">{t("reviewChanges")}')
