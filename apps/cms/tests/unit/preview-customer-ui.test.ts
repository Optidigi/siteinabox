...
describe("customer preview UI", () => {
  it("uses the dedicated customer preview renderer without editor metadata badges", () => {
    ...
    expect(customizer).toContain("pointer-events-auto relative isolate grid w-full grid-cols-[auto_1fr] items-center gap-1 overflow-hidden")
    expect(customizer).toContain("ShineBorder")
    expect(customizer).toContain("rgba(255,255,255,0.95)")
    expect(customizer).not.toContain('"white"')
