...
describe("customer preview UI", () => {
  it("uses a dedicated customer preview canvas mode without editor metadata badges", () => {
    ...
    expect(customizer).toContain("grid-cols-[1fr_auto_1fr]")
    ...
    expect(customizer).toContain("pointer-events-auto grid w-full grid-cols-[1fr_auto_1fr] items-center gap-1")
    ...
    expect(customizer).toContain("md:px-3 md:py-2")
    expect(customizer).toContain("md:rounded-lg")
    expect(customizer).toContain('<span className="md:hidden" aria-hidden />')
