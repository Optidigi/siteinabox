...
describe("shared email template safety", () => {
  ...
  it("matches the public Site in a Box typography and primary CTA styling", () => {
    ...
    expect(emailTheme.headingWeight).toBe(600)
    expect(emailTheme.buttonFont).toBe("Chivo,sans-serif")
    expect(message.html).toContain("font-family:'Familjen Grotesk Email','Familjen Grotesk',system-ui,-apple-system,sans-serif;font-size:26px;font-weight:600;letter-spacing:-.01em")
    expect(message.html).toContain("font-family:'Familjen Grotesk Email';font-style:normal;font-weight:600")
    expect(message.html).toContain("Qw3LZR9ZHiDnImG6-NEMQ41wby8WRnYsfkunR_eGfMG7aRzqeVk.woff2")
