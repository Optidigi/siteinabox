...
describe("PageForm wiring boundaries", () => {
  ...
  it("keeps official Amicare canvas parity while moving customer preview to an iframe", () => {
    ...
    expect(canvasSurface).toContain('tenantRendererKey === "amicare"')
    expect(canvasSurface).toContain("effectiveTenantCss")
    expect(canvasSurface).toContain("tenantRendererKey ? null : tenantCss")
    expect(canvasSurface).toContain("const rendererTheme = React.useMemo(() => normalizeThemeForSave(theme), [theme])")
    expect(canvasSurface).toContain("theme={rendererTheme}")
    expect(canvasSurface).not.toContain("themeCss")
    ...
    expect(canvasSurface).toContain("renderHeader={!showChrome")
    expect(canvasSurface).toContain("renderHeaderChrome(defaultChrome)")
    expect(canvasSurface).toContain("renderFooter={!showChrome")
    expect(canvasSurface).toContain("renderFooterChrome(defaultChrome)")
