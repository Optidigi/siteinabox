...
describe("editor frame route source contract", () => {
  ...
  it("defines an authenticated editor-frame page route with tenant ownership checks", () => {
    ...
    expect(route).toContain('import { getTenantById, getTenantBySlug } from "@/lib/queries/tenants"')
    expect(route).toContain('import { relationshipId, sameRelationshipId } from "@/lib/relationshipId"')
    ...
    expect(route).toContain("parseInitialView")
    expect(route).toContain("parseInitialMobileMode")
    expect(route).toContain('searchParams.mobileMode !== "fullPage" && searchParams.mobileMode !== "focusedSection"')
    expect(route).toContain("parseBooleanParam")
    expect(route).toContain("initialView={initialView}")
    expect(route).toContain("initialMobileMode={initialMobileMode}")
    expect(route).toContain('ctx.mode === "tenant"')
    expect(route).toContain('ctx.mode !== "super-admin"')
    expect(route).toContain("if (!tenantSlugParam && page) {")
    expect(route).toContain("const tenantId = relationshipId(page.tenant)")
    expect(route).toContain("const tenant = await getTenantById(tenantId).catch(() => null)")
    expect(route).toContain('const isNewPage = id === "new"')
    expect(route).toContain("if (!isNewPage && !Number.isFinite(pageId)) notFound()")
    expect(route).toContain("const page = isNewPage ? null : await getPageById(pageId!).catch(() => null)")
    expect(route).toContain("if (!isNewPage && !page) notFound()")
