...
describe("FE-80 save status UI", () => {
  it("uses the registry-owned merged save status primitives", () => {
    ...
    expect(statusBar).toContain('layout?: "viewport" | "canvas"')
    expect(statusBar).toContain('layout = "viewport"')
    expect(statusBar).toContain('layout === "canvas"')
    expect(statusBar).toContain("save-status-bar-canvas")
    expect(pageForm).toContain('layout="canvas"')
    expect(pageForm).toContain("page-editor-save-status-bar-position")
