...
describe("preview mobile chrome", () => {
  it("pairs edit left of launch at bottom-right with a left-aligned theme bar", () => {
    ...
    expect(chrome).not.toContain("consentAvailable")
    expect(chrome).not.toContain('data-mobile-preview-consent')
    expect(chrome).not.toContain('verticalOffset="4rem"')
    expect(chrome).not.toContain('verticalOffset="8rem"')
