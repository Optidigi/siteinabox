...
describe("preview customizer source contract", () => {
  it("hosts customer preview through an embedded renderer iframe contract", () => {
    ...
    expect(componentSource).toContain("pointer-events-auto w-full shadow-lg md:w-auto md:rounded-lg md:shadow-none")
    expect(componentSource).toContain("grid w-full grid-cols-[auto_1fr] items-center gap-1 border-t bg-background px-3 py-2 md:inline-flex md:w-auto md:grid-cols-none md:items-center md:gap-3 md:border-0 md:p-3")
    ...
    expect(componentSource).toContain("md:rounded-lg md:shadow-none")
    ...
    expect(componentSource).toContain("rgba(255,255,255,0.95)")
    expect(componentSource).toContain("contentClassName")
    const shineSource = fs.readFileSync(path.resolve(process.cwd(), "src/components/common/shine-border.tsx"), "utf8")
    expect(shineSource).not.toContain("mask-composite:exclude")
    expect(shineSource).not.toContain("-webkit-mask-composite")
    expect(shineSource).not.toContain("will-change-[background-position]")
    expect(shineSource).toContain("children")
    expect(shineSource).toContain("shine-border-lid")
