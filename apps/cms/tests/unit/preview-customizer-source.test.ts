...
describe("preview customizer source contract", () => {
  it("hosts customer preview through an embedded renderer iframe contract", () => {
    ...
    expect(componentSource).toContain("className=\"grid w-full grid-cols-[1fr_auto_1fr] items-center gap-4\"")
    expect(componentSource).toContain("h-12 justify-self-start")
    expect(componentSource).toContain("h-12 justify-self-end")
