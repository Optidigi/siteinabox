...
describe("preview customizer source contract", () => {
  it("hosts customer preview through an embedded renderer iframe contract", () => {
    ...
    expect(componentSource).toContain("pointer-events-auto relative isolate grid w-full grid-cols-[auto_1fr] items-center gap-1 overflow-hidden")
    ...
    expect(componentSource).toContain("md:rounded-lg md:border-0 md:bg-background md:p-3 md:shadow-none")
    ...
    expect(componentSource).not.toContain('"white"')
    expect(fs.readFileSync(path.resolve(process.cwd(), "src/components/common/shine-border.tsx"), "utf8")).not.toContain(
