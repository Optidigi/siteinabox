...
describe("preview customizer source contract", () => {
  it("hosts customer preview through an embedded renderer iframe contract", () => {
    ...
    expect(componentSource).toContain("size-12 justify-self-start")
    ...
    expect(componentSource).toContain('<span className="sr-only">{t("reviewChanges")}</span>')
    expect(componentSource).not.toContain('md:inline">{t("reviewChanges")}')
