...
`ThemeBar` (`src/components/editor/theme/theme-bar.tsx`) is a floating glass pill with four segments (`Colours | Fonts | Shape | Density`) controlled by a Radix `ToggleGroup type="single"` + `Popover` + `PopoverAnchor`. Each segment opens a panel anchored under it. The toolbar exposes only fixed ThemeTokenSpec V2 presets: five color styles (`Tailwind Native`, `Slate Indigo`, `Blue Professional`, `Emerald Calm`, `Amber Warm`), four font styles (`Clear Modern`, `Classic Editorial`, `Friendly Organic`, `Bold Confident`), four shape styles (`Tailwind Native`, `Sharp`, `Soft`, `Rounded`), and four density styles (`Tailwind Native`, `Compact`, `Comfortable`, `Spacious`). The default is one of those presets in each group, not an extra generated option. The bar edits **tenant-wide** preset IDs only — never CMS shadcn tokens, raw palettes, raw font families, raw radius values, arbitrary classes, or custom CSS. The schema stores ThemeTokenSpec V2 directly: `appearance.mode`, `colors.schemeId`, `fonts.schemeId`, `shape.schemeId`, and `density.schemeId`. `toCssVars` resolves those preset IDs into a base `.rt-canvas { ... }` block and a `.rt-canvas[data-rt-mode="dark"] { ... }` overlay; `CanvasSurface` stamps `data-rt-mode` from `theme.appearance.mode`. `packages/site-renderer` emits the same contract through `ThemeStyle`, `themeToCssVars`, and `data-rt-mode` on live/preview renderer roots. Tailwind's native `dark:` variant is wired to `[data-rt-mode="dark"]` in the generated-site CSS entrypoints. Tailwind Plus provider DOM, classes, layout, and responsive behavior remain renderer/source owned; theme presets affect only the approved bridge variables for color ramps, fonts, radius scale, and coarse section rhythm. In the page editor, changes update `PageForm`'s live `theme` state and persist through the normal explicit page save. In customer preview, theme changes go through the preview customizer's queued preview-theme save path. Both paths are validated by `themeSchema`.
