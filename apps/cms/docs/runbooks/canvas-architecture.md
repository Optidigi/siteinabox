...
`ThemeBar` (`src/components/editor/theme/theme-bar.tsx`) is a floating desktop control for the approved `ThemeTokenSpec` V3 color, font, shape, and appearance presets. It edits tenant-wide preset IDs only—never raw palettes, arbitrary classes, or custom CSS. A canonical manifest generates one static scoped preset stylesheet used by canvas, preview, and public rendering. `ThemeCanvas` selects presets with color/font/shape attributes and keeps the configured preference (`light`, `dark`, or `system`) separate from its resolved `data-rt-mode`; system mode follows the operating-system media query. A theme-only editor change sends one `theme.patch` and updates those attributes without replacing the page or rerendering its block tree. Public pages use the same attributes plus the optional `siab-color-mode` visitor override, applied by an inline head bootstrap before CSS paints. Live provider surfaces inherit the selected semantic tokens under `data-provider-token-mode="theme"`. Changes persist through the normal page or preview-theme save path.
