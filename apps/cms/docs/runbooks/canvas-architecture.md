...
`ThemeBar` (`src/components/editor/theme/theme-bar.tsx`) is a floating glass pill with four segments (`Colours | Fonts | Shape | Density`) controlled by a Radix `ToggleGroup type="single"` + `Popover` + `PopoverAnchor`. Each segment opens a panel anchored under it (no inline shape-morph). Sub-controls: `PalettePicker` shows six preset families, each with a light and dark half; clicking a half applies that palette and flips `theme.mode`, and the `+ Custom` popover has Light/Dark tabs for editing the active half. `FontPicker` shows Generated Style plus four named preset families (`Clear Modern`, `Classic Editorial`, `Friendly Organic`, `Bold Confident`); font values are normalized to the renderer-loaded families (`Inter Variable`, `Fraunces Variable`, `Caveat Variable`) before save. `ShapeControl` shows four shape choices (`Rounded`, `Soft`, `Subtle`, `Square`) and maps them to tenant-wide radius tokens. `DensityControl` exposes the finite contract choices (`Compact`, `Comfortable`, `Spacious`) as coarse tenant-wide section rhythm; it does not expose arbitrary spacing classes or per-block layout controls. Border style and style preset remain in the schema but are not surfaced as separate controls. The bar edits **tenant-wide** design tokens — never the CMS's own shadcn tokens — seeded from `FONT_PRESETS`, `PALETTE_PRESETS`, `RADIUS_PRESETS`, and `DENSITY_PRESETS` in `src/lib/theme/presets.ts`. The schema stores `palette` (light), `darkPalette`, `fonts.{title,heading,text,script}`, `radius`, `borderStyle`, `density`, `stylePreset`, and `mode: "light"|"dark"`. `toCssVars` emits a base `.rt-canvas { ... }` block plus, when `darkPalette` is set, a `.rt-canvas[data-rt-mode="dark"] { ... }` overlay block; `CanvasSurface` stamps `data-rt-mode` from `theme.mode` so the overlay wins. `packages/site-renderer` emits the same theme contract through `ThemeStyle`, `themeToCssVars`, and `data-rt-mode` on live/preview renderer roots. Tailwind's native `dark:` variant is wired to `[data-rt-mode="dark"]` in the generated-site CSS entrypoints, but active Tailwind Plus provider sources are mostly light-only. Their current theme behavior comes from renderer-owned bridge rules under provider root markers that map utility roles to SiaB tokens without globally redefining neutral Tailwind palette variables. The role tokens consumed by block renderers + `InlineCtaButton` are `var(--font-{title,heading,text,script})`, `var(--radius-{sm,md,lg})` (sm = radius - 0.25rem clamped at 0; lg = radius + 0.5rem), and coarse density/rhythm variables such as `--site-section-padding-y`. In the page editor, changes update `PageForm`'s live `theme` state and persist through the normal explicit page save. In customer preview, theme changes go through the preview customizer's queued preview-theme save path. Both paths are validated by `themeSchema`.
...
Fonts, colors, shape, radius, border style, mode, and coarse density/rhythm are
