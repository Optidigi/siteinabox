...
export function CheckoutStepper<T extends string = string>({
  ...
  return (
    ...
    >
      <div className="grid min-h-11 min-w-0 grid-cols-[auto_minmax(3rem,1fr)] items-center gap-3 rounded-xl border bg-card/70 px-3 min-[880px]:hidden">
        <span className="min-w-0 truncate text-xs font-semibold">{progressText(activeIndex + 1, steps.length, steps[activeIndex]?.label ?? "")}</span>
        ...
        {steps.map((entry, index) => {
          ...
          const content = (
            <>
              ...
              <span className="min-w-0 truncate text-left text-[0.6875rem] font-semibold leading-tight min-[560px]:text-[0.8125rem]">
                {entry.label}
              </span>
            </>
          )
