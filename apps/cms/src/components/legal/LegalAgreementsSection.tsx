...
export async function LegalAgreementsSection({
  requirements,
  acceptanceHistory,
  locale,
  result,
  ...
  const t = await getTranslations("legalOperations.agreements")

  return (
    <Card id="agreements" className="w-full max-w-3xl scroll-mt-20">
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-xl"><FileCheck2 className="size-5" /> {t("title")}</CardTitle>
        <CardDescription>{t("description")}</CardDescription>
      </CardHeader>
      <CardContent className="grid gap-5">
        {result === "accepted" && (
          <Alert><CheckCircle2 /><AlertTitle>{t("acceptedTitle")}</AlertTitle><AlertDescription>{t("acceptedDescription")}</AlertDescription></Alert>
        )}
        {result === "objected" && (
          <Alert><CheckCircle2 /><AlertTitle>{t("objectedTitle")}</AlertTitle><AlertDescription>{t("objectedDescription")}</AlertDescription></Alert>
        )}
        {result === "failed" && (
          <Alert variant="destructive"><AlertTitle>{t("failedTitle")}</AlertTitle><AlertDescription>{t("failedDescription")}</AlertDescription></Alert>
        )}
        {result === "acceptance-required" && (
          <Alert variant="destructive"><AlertTitle>{t("confirmationRequiredTitle")}</AlertTitle><AlertDescription>{t("confirmationRequiredDescription")}</AlertDescription></Alert>
        )}

        {!requirements.length && (
          <div className="flex items-start gap-3 rounded-md border p-4">
            <CheckCircle2 className="mt-0.5 size-5 text-muted-foreground" />
            <div><p className="font-medium">{t("upToDateTitle")}</p><p className="text-sm text-muted-foreground">{t("upToDateDescription")}</p></div>
          </div>
        )}
          ...
          <section key={requirement.requirementKey} className="grid gap-4 rounded-md border p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="grid gap-1">
                <h3 className="font-medium">{t("termsTitle")}</h3>
                <p className="text-sm text-muted-foreground">
                  {requirement.isInitialRelease
                    ? t("initialTermsDescription")
              ...
              >
                {requirement.action === "notice_and_continued_use" ? t("noticeBadge") : t("acceptanceRequiredBadge")}
              </Badge>
            </div>
            <p className="text-sm text-muted-foreground">
              {t("effectiveFrom", { date: formatDate(requirement.effectiveAt, locale) })}
              {requirement.objectionDeadlineAt ? t("objectionUntil", { date: formatDate(requirement.objectionDeadlineAt, locale) }) : requirement.enforceAt ? t("acceptanceBefore", { date: formatDate(requirement.enforceAt, locale) }) : ""}
            </p>
            <Button asChild variant="outline" className="w-fit">
              <Link href={requirement.href} target="_blank" rel="noopener noreferrer">
                {t("viewDocument")} <ExternalLink className="size-4" />
              </Link>
            </Button>
            <form action={acceptLegalRequirementAction} className="grid gap-4 border-t pt-4">
              ...
              {requirement.requiresAcceptance ? (
                <label className="flex items-start gap-3 text-sm leading-6">
                  ...
                  <span>{t("acceptanceConfirmation")}</span>
                </label>
              ) : <input type="hidden" name="acceptance" value="accepted" />}
              <div className="flex flex-wrap items-center gap-2">
                <Button type="submit" className="w-fit">{t("accept")}</Button>
                {requirement.canObject && (
                  <Button formAction={objectLegalRequirementAction} name="objection" value="confirmed" type="submit" variant="outline">{t("recordObjection")}</Button>
                )}
              </div>
            </form>
          </section>
        ...
        {notices.map((requirement) => (
          <section key={requirement.requirementKey} className="flex flex-wrap items-start justify-between gap-4 rounded-md border p-4">
            <div className="grid gap-1"><h3 className="font-medium">{t("legalNotice")}</h3><p className="text-sm text-muted-foreground">{requirement.changeSummary}</p></div>
            <Button asChild size="sm" variant="outline"><Link href={requirement.href}>{t("view")}</Link></Button>
          </section>
        ))}
        {acceptanceHistory.length > 0 && (
          <Collapsible className="group border-t pt-5">
            <CollapsibleTrigger className="flex w-full items-center justify-between gap-3 rounded-md px-1 py-2 text-left font-medium outline-none hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
              <span>{t("history")} ({acceptanceHistory.length})</span>
            ...
            <CollapsibleContent className="pt-3">
              ...
              {acceptanceHistory.map((acceptance) => (
                <div key={acceptance.id} className="flex flex-wrap items-center justify-between gap-3 p-3 text-sm">
                  <div>
                    <p className="font-medium">{t("termsVersion", { version: acceptance.documentVersion })}</p>
                    <p className="text-muted-foreground">{formatDate(acceptance.acceptedAt, locale)} · {acceptance.actorEmail}</p>
                  </div>
                  <Button asChild size="sm" variant="ghost"><Link href={acceptance.href} target="_blank" rel="noopener noreferrer">{t("view")}</Link></Button>
                </div>
              ))}
              </div>
            </CollapsibleContent>
