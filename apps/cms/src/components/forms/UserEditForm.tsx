...
type EditableRole = "super-admin" | "owner" | "editor" | "viewer"
...
export function UserEditForm({
  ...
  user: User
  tenants: TenantLite[]
  operator: { id: number | string; role: EditableRole }
}) {
  ...
  const isSelf = String(operator.id) === String(user.id)
  const roleOptions: EditableRole[] =
                  ...
                  <FormField name="tenantId" control={form.control} render={({ field }) => (
                    <FormItem>
                      ...
                      <Select
                        ...
                        disabled={operator.role !== "super-admin" && tenants.length <= 1}
                      >
