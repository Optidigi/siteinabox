...
export default async function LegalRequirementDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="flex flex-col gap-4">
    <PageHeader title={t("requirementDetail.title")} subtitle={recordLabel(record.tenant, ["name", "slug"]) || t("unknownTenant")} beforeTitle={<Button asChild variant="ghost" size="sm"><Link href="/legal/requirements"><ArrowLeft />{t("detail.back")}</Link></Button>} />
    <LegalRouteTabs activePath="/legal/requirements" />
    <LegalRecordDetail title={t("requirementDetail.recordTitle")} fields={[
      { label: t("fields.tenant"), value: recordLabel(record.tenant, ["name", "slug", "domain"]) }, { label: t("fields.account"), value: record.subjectEmail },
      { label: t("fields.document"), value: recordLabel(record.document, ["title", "releaseKey"]) }, { label: t("fields.action"), value: record.action },
      ...
      { label: t("fields.acceptanceEvidence"), value: recordLabel(record.acceptance, ["evidenceKey", "id"]) }, { label: t("fields.requirementKey"), value: record.requirementKey, mono: true },
