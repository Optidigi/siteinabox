...
export const chromeDraftFromSettings = (
  ...
  // Banner/cookie copy is owned by Settings; draft only passthroughs for chrome saves.
...
export const mergeChromeSettings = (settings: SiteSetting | null | undefined, draft: SiteChromeDraft) => ({
  ...
  chrome: {
    ...
    // Passthrough only — cookie/announcement copy is edited in Settings.
    banner: draft.banner ?? settings?.chrome?.banner,
  },
})
...
export const rendererSettingsFromChromeDraft = (
  ...
  return settingsToJsonWithoutAnalytics(
