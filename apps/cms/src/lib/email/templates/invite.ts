...
export function inviteTemplate(opts: {
  ...
  return {
    ...
    html: renderEmailLayout({
      ...
      body: `<p style="margin:0 0 14px;font-family:${emailTheme.bodyFont};font-size:14px;line-height:1.55">Je rol: <strong>${escapeEmailHtml(roleLabel)}</strong></p>${renderEmailButton("Uitnodiging accepteren", opts.inviteUrl)}${renderEmailFallbackLink(opts.inviteUrl)}`,
