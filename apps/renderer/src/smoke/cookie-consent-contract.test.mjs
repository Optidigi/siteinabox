...
test("renderer keeps only structural consent and transport guards here", () => {
  ...
  assert.match(runtimeSource, /sealPostHogTransportAfterRevoke/)
  assert.match(runtimeSource, /__siabConsentGated/)
  assert.match(runtimeSource, /_isPolling/)
  assert.doesNotMatch(runtimeSource, /queue\.unload/)
