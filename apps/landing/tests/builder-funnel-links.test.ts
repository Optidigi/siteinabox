...
describe("public builder funnel links", () => {
  it("sends production Start gratis and Inloggen to admin.siteinabox.nl/login, not localhost", () => {
    ...
    expect(config).toContain("destination: 'https://admin.siteinabox.nl/login'")
    expect(config).toContain("destination: 'https://admin.siteinabox.nl/login?intent=register'")
  })
})
