...
async function setTheme(page: import('playwright/test').Page, theme: 'light' | 'dark') {
  await page.addInitScript((value) => localStorage.setItem('siab-theme', value), theme);
}
...
test('interactive controls remain accessible', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Thema: licht (klik voor donker)' }).click();
  await expect(page.locator('html')).toHaveAttribute('data-theme', 'dark');
  await page.reload();
  const reloadedThemeTrigger = page.getByRole('button', { name: 'Thema: donker (klik voor licht)' });
  await waitForIsland(reloadedThemeTrigger);
  await expect(page.locator('html')).toHaveAttribute('data-theme', 'dark');

  const yearlySwitch = page.getByRole('switch', { name: 'Jaarlijkse facturatie' });
  ...
  const tooltipTrigger = page.getByRole('button', { name: /Meer info: Homepage/ });
  await tooltipTrigger.focus();
  await expect(page.getByRole('tooltip')).toBeVisible();

  const carousel = page.getByRole('region', { name: 'Klantbeoordelingen' });
  await waitForIsland(carousel);
  const beforeTransform = await carousel.locator('[data-slot="carousel-content"] > div').evaluate(
    (element) => getComputedStyle(element).transform,
  );
  await page.getByRole('button', { name: 'Next slide' }).click();
  await expect.poll(async () => carousel.locator('[data-slot="carousel-content"] > div').evaluate(
    (element) => getComputedStyle(element).transform,
  )).not.toBe(beforeTransform);
  await carousel.focus();
  const beforeKeyboardTransform = await carousel.locator('[data-slot="carousel-content"] > div').evaluate(
    (element) => getComputedStyle(element).transform,
  );
  ...
  const question = page.getByRole('button', { name: 'Hoe weten jullie wat er op mijn website moet komen?' });
  await waitForIsland(question);
  await question.click();
  await expect(question).toHaveAttribute('aria-expanded', 'true');

  const marqueeDuration = await page.locator('.siab-marquee-track').evaluate(
...
test('compact header and process cards keep their responsive geometry', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Open navigatie' })).toBeVisible();
  ...
  const visualHeights = await page.locator('[data-process-visual]').evaluateAll(
    (elements) => elements.map((element) => Math.round(element.getBoundingClientRect().height)),
  );
  expect(new Set(visualHeights).size).toBe(1);
});

test('desktop bento preserves the production clipping and process form containment', async ({ page }) => {
  ...
  expect(photosBox!.y + photosBox!.height).toBeLessThanOrEqual(formBox!.y + formBox!.height);
  expect(await page.locator('[data-process-form]').evaluate(
  ...
  await expect.poll(() => phoneScreen.locator('img').evaluate(
    (image: HTMLImageElement) => image.complete && image.naturalWidth > 0,
  )).toBe(true);
  await expect(safariScreen.locator('img')).toHaveCSS('object-position', '50% 0%');
  await expect(safariScreen).toHaveCSS('overflow', 'hidden');
  await expect(phoneScreen).toHaveCSS('overflow', 'hidden');
  expect(await phoneScreen.evaluate((element) => getComputedStyle(element).borderRadius)).not.toBe('0px');

  const deviceCard = page.locator('#zo-werkt-het article').nth(1);
  const phoneFrameBox = await page.locator('[data-device-phone-frame]').boundingBox();
  const safariFrameBox = await page.locator('[data-device-safari-frame]').boundingBox();
  ...
  await expect(page.locator('[data-device-stack]')).toHaveCSS('margin-bottom', '-80px');
  expect(await page.locator('[data-device-phone-frame]').evaluate(
    (element) => Number.parseFloat(getComputedStyle(element).bottom),
  )).toBeLessThan(0);
  expect(phoneFrameBox!.y + phoneFrameBox!.height).toBeGreaterThanOrEqual(cardBox!.y + cardBox!.height - 2);
  expect(safariFrameBox!.y + safariFrameBox!.height).toBeGreaterThanOrEqual(cardBox!.y + cardBox!.height - 2);

  const bentoType = await page.locator('#zo-werkt-het article h3').evaluateAll(
    (elements) => elements.map((element) => getComputedStyle(element).fontSize),
  );
  expect(bentoType).toEqual(['35px', '35px', '28px']);
  await expect(page.locator('#zo-werkt-het blockquote')).toHaveCSS('font-size', '28px');
  await expect(page.locator('#zo-werkt-het article').first().locator('svg')).toHaveCSS('color', 'rgb(255, 255, 255)');
  await expect(page.locator('#zo-werkt-het article').last().locator('svg')).toHaveCSS('color', 'rgb(255, 255, 255)');

  const reviewCardBox = await page.locator('#zo-werkt-het article').nth(2).boundingBox();
...
test('contact and beheer validation contracts remain intact', async ({ page }) => {
  await page.goto('/contact');
  await page.getByRole('button', { name: 'Verstuur bericht' }).click();
  await expect(page.getByText('Vul je naam in zodat we weten met wie we praten.')).toBeVisible();
  await expect(page.locator('#siab-name')).toHaveAttribute('aria-invalid', 'true');

  await page.goto('/beheer');
  await page.locator('#tenant-domain').fill('geen geldig domein');
  await page.getByRole('button', { name: 'Inloggen' }).click();
  await expect(page.getByText('Vul een geldig domein in, bijvoorbeeld ami-care.nl.')).toBeVisible();
  await expect(page.locator('#tenant-domain')).toBeFocused();
});

test('contact success and failure states remain actionable', async ({ page }) => {
  const fillContactForm = async () => {
    ...
    await page.locator('#siab-message').fill('Dit is een testbericht voor de browsercontrole.');
    await page.locator('#siab-consent').check();
    await expect(page.locator('[name="cf-turnstile-response"]')).toHaveValue('XXXX.DUMMY.TOKEN.XXXX');
  };
  ...
  await fillContactForm();
  await page.getByRole('button', { name: 'Verstuur bericht' }).click();
  await expect(page.locator('[data-siab-turnstile-error]')).toContainText('beveiligingscontrole is verlopen');
...
test('beheer redirects valid tenant domains and public routing remains intact', async ({ page }) => {
  await page.route('https://admin.ami-care.nl/login', (route) => route.fulfill({
    status: 200,
  ...
  await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href', /privacy-en-cookieverklaring/);
