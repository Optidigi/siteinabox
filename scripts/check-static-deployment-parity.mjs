...
for (const [label, expected] of staticDockerContracts) present(`landing ${label}`, files.landingDocker, expected);

present("landing Nginx config copy", files.landingDocker, "COPY apps/landing/nginx.conf /etc/nginx/conf.d/default.conf");
present("landing static output copy", files.landingDocker, "COPY --from=build /repo/apps/landing/dist /usr/share/nginx/html");
...
absent("CMS compose must not expose public intake API", dataDirFiles.cmsCompose, "PathPrefix(`/api/intake`)");
