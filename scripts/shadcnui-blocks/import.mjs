...
const runtimeVariant = (variant) => ({
  id: variant.id,
  upstreamName: variant.upstreamName,
  title: variant.title,
  description: variant.description,
  role: variant.role,
  ...(variant.blockType ? { blockType: variant.blockType } : {}),
  ...(variant.area ? { area: variant.area } : {}),
  ...(variant.kind ? { kind: variant.kind } : {}),
  activeSlots: Object.fromEntries(
    Object.entries(variant.slots).filter(([, slot]) => slot.status !== "inactive"),
  ),
  forbiddenFields: Object.entries(variant.slots)
...
export async function emitRuntimeCatalog(sourceInventory) {
  const inventory = sourceInventory
    ?? JSON.parse(await readFile(join(providerRoot, "inventory.json"), "utf8"))
  const blockVariants = inventory.variants.filter((variant) => variant.role === "block")
  const systemBlockVariants = [{
    id: `${NAMESPACE}.legal-content-01`, upstreamName: "legal-content-01", title: "Legal content", description: "Provider-styled structured long-form legal content.", role: "block", blockType: "contentSection",
    slots: Object.fromEntries(allStructuredFields.map((field) => [field, field === "body" ? { kind: "richtext", status: "required", repeated: false } : { kind: fieldSets.contentSection[field]?.[0] ?? "inactive", status: "inactive", repeated: fieldSets.contentSection[field]?.[2] ?? false }])),
    composition: { embedsNavigation: false, suppressesChromeAreas: [] },
  }]
  ...
  await writeFile(join(generatedRoot, "shadcnui-blocks.ts"), [
    ...
    `export const SHADCNUI_PROVIDER = ${JSON.stringify({ provider: inventory.provider, namespace: inventory.namespace, repository: inventory.repository, commit: inventory.commit, registry: inventory.registry, registryHash: inventory.registryHash, license: inventory.license, counts: inventory.counts }, null, 2)} as const`,
    `export const SHADCNUI_BLOCK_VARIANTS = ${JSON.stringify(blockVariants.map(runtimeVariant), null, 2)} as const`,
    `export const SHADCNUI_CHROME_VARIANTS = ${JSON.stringify(inventory.variants.filter((variant) => variant.role === "chrome").map(runtimeVariant), null, 2)} as const`,
    `export const SHADCNUI_SYSTEM_TEMPLATES = ${JSON.stringify(inventory.variants.filter((variant) => variant.role === "systemTemplate").map(runtimeVariant), null, 2)} as const`,
    `export const SHADCNUI_SYSTEM_BLOCK_VARIANTS = ${JSON.stringify(systemBlockVariants.map(runtimeVariant), null, 2)} as const`,
...
`
    ...
    await emitRuntimeCatalog(inventory)
