// Owned typed adaptation of upstream shadcnui-blocks team-07 (MIT, see ../../LICENSE).
"use client"
...
const MAX_MEMBERS = 8
export type Team07Props = TypedVariantBaseProps & { title?: RtRoot | null; intro?: RtRoot | null; members: TeamMemberItem[]; mediaResolver?: MediaResolver }
export function Team07({ title, intro, members, blockIndex, editSlots, mediaResolver, rootAttributes }: Team07Props) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-20" {...rootAttributes}>
      ...
      <div className="mt-12 grid grid-cols-1 sm:mt-16 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {displayMembers.map((member, itemIndex) => (
          <div className="-mt-px -ml-px border py-8 border-border" key={itemIndex}>
            ...
            <p className="mt-4 text-center font-medium text-lg">{renderMemberName(editSlots, member.name, blockIndex, itemIndex)}</p>
...
export default function Team07Literal(){return(<div className="mx-auto max-w-7xl px-6 py-20"><h2 className="text-balance text-center font-medium text-3xl tracking-[-0.04em] sm:text-4xl md:text-[2.75rem]"><RichTextRenderer value={teamFamilyCmsLike.title} blockMode="inline"/></h2><p className="mt-3 text-balance text-center text-lg text-muted-foreground tracking-[-0.01em] md:text-2xl"><RichTextRenderer value={teamFamilyCmsLike.intro} blockMode="inline"/></p><div className="mt-12 grid grid-cols-1 sm:mt-16 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">{F.map((m,i)=>(<div className="-mt-px -ml-px border py-8 border-border" key={i}><div className="mx-auto aspect-square max-w-48 overflow-hidden rounded-full bg-muted"><img alt={m[0]} className="h-full w-full object-cover" src={I[i]}/></div><p className="mt-4 text-center font-medium text-lg">{m[0]}</p><p className="mt-0.5 text-center text-muted-foreground">{m[1]}</p><div className="mt-5 flex items-center justify-center gap-4"><a href="#"><Twitter className="h-5 w-5 text-muted-foreground hover:text-primary"/></a><a href="#"><Github className="h-5 w-5 text-muted-foreground hover:text-primary"/></a><a href="#"><Dribbble className="h-5 w-5 text-muted-foreground hover:text-primary"/></a></div></div>))}</div></div>)}
