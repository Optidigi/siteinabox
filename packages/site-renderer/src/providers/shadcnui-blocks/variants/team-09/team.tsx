    ...
    <div className="mx-auto max-w-7xl px-6 py-20" {...rootAttributes}>
      ...
      <div className="mt-12 grid grid-cols-1 gap-4 sm:mt-16 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {displayMembers.map((member, itemIndex) => (
          <div className="border bg-muted py-8" key={itemIndex}>
...
export default function Team09Literal(){return(<div className="mx-auto max-w-7xl px-6 py-20"><h2 className="text-balance text-center font-medium text-3xl tracking-[-0.04em] sm:text-4xl md:text-[2.75rem]"><RichTextRenderer value={team06CmsLike.title} blockMode="inline"/></h2><p className="mt-3 text-balance text-center text-lg text-muted-foreground tracking-[-0.01em] md:text-2xl"><RichTextRenderer value={team06CmsLike.intro} blockMode="inline"/></p><div className="mt-12 grid grid-cols-1 gap-4 sm:mt-16 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">{F.map((m,i)=>(<div className="border bg-muted py-8" key={i}><div className="mx-auto aspect-square max-w-48 overflow-hidden rounded-full bg-muted"><img alt={m[0]} className="h-full w-full object-cover" src={I[i]}/></div><p className="mt-6 text-center font-medium text-lg">{m[0]}</p><p className="mt-0.5 text-center text-muted-foreground">{m[1]}</p><div className="mt-6 flex items-center justify-center gap-4"><a href="#"><Twitter className="h-5 w-5 text-muted-foreground hover:text-primary"/></a><a href="#"><Github className="h-5 w-5 text-muted-foreground hover:text-primary"/></a><a href="#"><Dribbble className="h-5 w-5 text-muted-foreground hover:text-primary"/></a></div></div>))}</div></div>)}
