// Owned typed adaptation of upstream shadcnui-blocks hero-04 (MIT, see ../../LICENSE).
"use client"
...
import { hero04LiteralWithImage } from "../../typed/fixtures/hero-family"
import type { TypedVariantBaseProps } from "../../typed/props"

export type Hero04Props = TypedVariantBaseProps & {
  eyebrow?: RtRoot | null
  headline: RtRoot
...
export function Hero04({
  ...
  ...rest
}: Hero04Props & React.HTMLAttributes<HTMLDivElement>) {
    ...
    <div className="flex min-h-screen items-center justify-center overflow-hidden" {...rootAttributes} {...rest}>
      <div className="mx-auto grid w-full max-w-(--breakpoint-xl) gap-12 px-6 py-12 lg:grid-cols-2 lg:py-0">
        ...
        {renderHeroImage(editSlots, mediaResolver, image, blockIndex, {
          className: "aspect-video w-full rounded-xl bg-accent lg:aspect-auto lg:h-[calc(100vh-4rem)] lg:w-[1000px]",
          literalPreview,
        })}
      </div>
    </div>
...
export default function Hero04Literal() {
  return (
    <Hero04
      eyebrow={hero04LiteralWithImage.eyebrow}
      headline={hero04LiteralWithImage.headline}
      subheadline={hero04LiteralWithImage.subheadline}
      cta={hero04LiteralWithImage.cta}
      secondary={hero04LiteralWithImage.secondary}
      blockIndex={0}
      literalPreview
    />
  )
