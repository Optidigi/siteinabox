// Owned typed adaptation of upstream shadcnui-blocks team-08 (MIT, see ../../LICENSE).
"use client"
...
const MAX_MEMBERS = 8
export type Team08Props = TypedVariantBaseProps & { title?: RtRoot | null; intro?: RtRoot | null; members: TeamMemberItem[]; mediaResolver?: MediaResolver }
export function Team08({ title, intro, members, blockIndex, editSlots, mediaResolver, rootAttributes }: Team08Props) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-20" {...rootAttributes}>
      {titleContent ? <h2 className="text-balance text-center font-medium text-3xl capitalize tracking-[-0.04em] sm:text-4xl md:text-[2.75rem]">{titleContent}</h2> : null}
      ...
      <div className="mt-12 grid grid-cols-1 gap-2 rounded-xl border border-border/75 bg-muted p-2 sm:mt-16 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 dark:border-border/60">
        {displayMembers.map((member, itemIndex) => (
          <div className="rounded-lg border border-border/85 bg-background py-8 dark:border-foreground/13" key={itemIndex}>
            <div className="mx-auto aspect-square max-w-48 overflow-hidden rounded-4xl bg-muted">{renderMemberImage(editSlots, mediaResolver, member.image, member.name, blockIndex, itemIndex, { alt: member.name, className: "h-full w-full object-cover" })}</div>
            <p className="mt-6 text-center font-medium text-lg">{renderMemberName(editSlots, member.name, blockIndex, itemIndex)}</p>
            <p className="text-center text-muted-foreground">{renderMemberRole(editSlots, member.role, blockIndex, itemIndex)}</p>
...
export default function Team08Literal(){return(<div className="mx-auto max-w-7xl px-6 py-20"><h2 className="text-balance text-center font-medium text-3xl capitalize tracking-[-0.04em] sm:text-4xl md:text-[2.75rem]"><RichTextRenderer value={team06CmsLike.title} blockMode="inline"/></h2><p className="mt-3 text-balance text-center text-lg text-muted-foreground tracking-[-0.01em] md:text-2xl"><RichTextRenderer value={team06CmsLike.intro} blockMode="inline"/></p><div className="mt-12 grid grid-cols-1 gap-2 rounded-xl border border-border/75 bg-muted p-2 sm:mt-16 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 dark:border-border/60">{F.map((m,i)=>(<div className="rounded-lg border border-border/85 bg-background py-8 dark:border-foreground/13" key={i}><div className="mx-auto aspect-square max-w-48 overflow-hidden rounded-4xl bg-muted"><img alt={m[0]} className="h-full w-full object-cover" src={I[i]}/></div><p className="mt-6 text-center font-medium text-lg">{m[0]}</p><p className="text-center text-muted-foreground">{m[1]}</p><div className="mt-5 flex items-center justify-center gap-4"><a href="#"><Twitter className="h-5 w-5 text-muted-foreground hover:text-primary"/></a><a href="#"><Github className="h-5 w-5 text-muted-foreground hover:text-primary"/></a><a href="#"><Dribbble className="h-5 w-5 text-muted-foreground hover:text-primary"/></a></div></div>))}</div></div>)}
