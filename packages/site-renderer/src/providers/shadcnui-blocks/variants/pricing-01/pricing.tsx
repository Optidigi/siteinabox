// Owned typed adaptation of upstream shadcnui-blocks pricing-01 (MIT, see ../../LICENSE).
"use client"
...
export type Pricing01Props = TypedVariantBaseProps & {
  title?: RtRoot | null
  intro?: RtRoot | null
  plans: PricingPlanItem[]
}

export function Pricing01({ title, intro, plans, blockIndex, editSlots, rootAttributes }: Pricing01Props) {
    ...
    <div className="px-6 py-20" {...rootAttributes}>
      ...
      <div className="mx-auto mt-12 grid max-w-(--breakpoint-lg) grid-cols-1 gap-8 sm:mt-16 lg:grid-cols-3">
        {displayPlans.map((plan, planIndex) => {
          ...
          return (
            <div className="rounded-lg border border-border/85 bg-card p-6 shadow-xs/3" key={planIndex}>
...
export default function Pricing01Literal() {
  return (
    <Pricing01
