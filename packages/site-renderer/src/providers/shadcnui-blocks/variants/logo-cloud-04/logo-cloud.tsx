...
import { logoCloud04Literal, logoCloudFamilyCmsLike } from "../../typed/fixtures/logo-cloud-family"
        ...
        <div className="mt-16 grid grid-cols-2 gap-6 md:grid-cols-3 lg:grid-cols-4">
          {literalPreview
            ? FALLBACK_LOGOS.map((Logo, itemIndex) => (
                <div
                  className="flex items-center justify-center rounded border border-dashed py-6 *:h-8 sm:px-10 sm:py-8 sm:*:h-10 border-border"
                  key={itemIndex}
                >
...
export default function LogoCloud04Literal() {
  return <LogoCloud04 intro={logoCloud04Literal.intro} logos={logoCloudFamilyCmsLike.logos} blockIndex={0} literalPreview />
}
