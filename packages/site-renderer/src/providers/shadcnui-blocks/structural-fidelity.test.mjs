...
test("hero-01 DreamyBackground keeps upstream blobs with CSS blur (no SVG feGaussianBlur)", async () => {
  ...
  assert.match(source, /contain:\s*"paint"/)
  assert.doesNotMatch(source, /feGaussianBlur/)
  assert.doesNotMatch(source, /bg-\[radial-gradient/)
  // translateZ belongs on the paint container / content, not on the filtered SVG.
  assert.match(source, /translateZ\(0\)/)
  assert.doesNotMatch(source, /translateZ\(0\) scale\(/)
})
