...
test("hero-01 DreamyBackground bakes upstream blobs to canvas (no live CSS/SVG blur)", async () => {
  const source = await readFile(new URL("./hero-01/hero.tsx", variantsRoot), "utf8")
  ...
  assert.match(source, /BLOB_BLUR_PX = 64/)
  assert.match(source, /BLOB_SCALE_X = 1\.58/)
  assert.match(source, /getContext\("2d"\)/)
  assert.match(source, /Path2D/)
  ...
  // Display path must not use a live CSS filter on SVG/DOM (canvas bake only).
  assert.doesNotMatch(source, /filter:\s*"blur\(64px\)"/)
  assert.doesNotMatch(source, /style=\{\{[^}]*filter:/)
})
