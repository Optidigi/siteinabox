...
test("themeToCssVars keeps Tailwind palette variables literal while bridging provider utility roles", () => {
  ...
  assert.match(css, /--color-tailwindplus-hero-glow-from:#ff80b5/)
  assert.match(css, /--color-tailwindplus-hero-glow-to:#9089fc/)
  assert.match(css, /--color-tailwindplus-source-glow-soft-from:#ff80b5/)
  assert.match(css, /--color-tailwindplus-source-glow-soft-to:#9089fc/)
  assert.match(css, /--color-tailwindplus-source-glow-vivid-from:#ff4694/)
  assert.match(css, /--color-tailwindplus-source-glow-vivid-to:#776fff/)
  ...
  assert.match(css, /:where\(\[data-provider-top-stack="tailwindplus"\]\.bg-white\)/)
...
test("themeToCssVars bridges active Tailwind Plus accent utilities without class changes", () => {
  ...
  assert.match(css, /--color-tailwindplus-hero-glow-from:#fca5a5/)
  assert.match(css, /--color-tailwindplus-hero-glow-to:#dc2626/)
  assert.match(css, /--color-tailwindplus-source-glow-soft-from:#fca5a5/)
  assert.match(css, /--color-tailwindplus-source-glow-soft-to:#dc2626/)
  assert.match(css, /--color-tailwindplus-source-glow-vivid-from:#f87171/)
  assert.match(css, /--color-tailwindplus-source-glow-vivid-to:#dc2626/)
  assert.match(css, /--color-tailwindplus-cta-radial-from:#f87171/)
  assert.match(css, /--color-tailwindplus-cta-radial-to:#dc2626/)
  assert.doesNotMatch(css, /--color-indigo-\d+:/)
  assert.match(css, /\[class~="shadow-indigo-600\/10"\][^}]*--tw-shadow-color:color-mix\(in oklab,var\(--color-accent,#4f46e5\) 10%,transparent\)/)
  assert.match(css, /\[class~="from-\[#ff80b5\]"\][^}]*--tw-gradient-from:var\(--color-tailwindplus-source-glow-soft-from,#ff80b5\)/)
  assert.match(css, /\[class~="to-\[#9089fc\]"\][^}]*--tw-gradient-to:var\(--color-tailwindplus-source-glow-soft-to,#9089fc\)/)
  assert.match(css, /\[class~="from-\[#ff4694\]"\][^}]*--tw-gradient-from:var\(--color-tailwindplus-source-glow-vivid-from,#ff4694\)/)
  assert.match(css, /\[class~="to-\[#776fff\]"\][^}]*--tw-gradient-to:var\(--color-tailwindplus-source-glow-vivid-to,#776fff\)/)
  assert.match(css, /\[data-siab-tokenized-gradient="hero-glow"\][^}]*--tw-gradient-from:var\(--color-tailwindplus-hero-glow-from,#ff80b5\)/)
  assert.match(css, /\[data-siab-tokenized-gradient="hero-glow"\][^}]*--tw-gradient-to:var\(--color-tailwindplus-hero-glow-to,#9089fc\)/)
  assert.match(css, /\[data-siab-tokenized-gradient="cta-radial"\]\) stop\[data-siab-gradient-stop="from"\][^}]*stop-color:var\(--color-tailwindplus-cta-radial-from,#7775D6\)/)
  assert.match(css, /\[data-siab-tokenized-gradient="cta-radial"\]\) stop\[data-siab-gradient-stop="to"\][^}]*stop-color:var\(--color-tailwindplus-cta-radial-to,#E935C1\)/)
