...
import { useCspStyleRule } from "../lib/csp-style"
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      {...props}
    >
      <PanelLeftIcon className="max-md:size-6" />
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      title={props.title ?? t("toggleSidebar")}
      className={cn(
...
const sidebarMenuButtonVariants = cva(
  // Active-state polish (round 3): font-semibold (was font-medium) for
  // stronger emphasis, plus a 4px-wide primary-colored accent bar on the
  // left edge via ::before. The `relative` is needed so the absolutely-
...
function SidebarMenuSkeleton({
  ...
  const widthRule = useCspStyleRule("sidebar-menu-skeleton", `--skeleton-width:${width};`)
  ...
  return (
    ...
    >
      ...
      {widthRule.styleElement}
      <Skeleton
        className={`${widthRule.className} h-4 max-w-(--skeleton-width) flex-1`}
